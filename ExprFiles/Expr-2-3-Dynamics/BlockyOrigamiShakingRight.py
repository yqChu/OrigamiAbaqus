...
for panel_id in range(len(panels_no_interval[:,0,0])):
    ...
    mdb.models['Model-1'].parts[f'Part-{panel_id+1}'].DatumCsysByThreePoints(name=f'Datum-csys-{panel_id+1}-C', coordSysType=CARTESIAN, origin=panels_interval[panel_id,0],
    point1=2*panels_interval[panel_id,0]-panels_interval[panel_id,1], point2=panels_interval[panel_id,2])

    mdb.models['Model-1'].parts[f'Part-{panel_id+1}'].DatumCsysByThreePoints(name=f'Datum-csys-{panel_id+1}-D', coordSysType=CARTESIAN, origin=panels_interval[panel_id,0],
...
for panel_id in range(len(panels_interval[:,0,0])):
    elements = mdb.models['Model-1'].parts[f'Part-{panel_id+1}'].elements.getSequenceFromMask(mask=('[#ffffffff #f ]',),)
...
for part_id in range(x_num*y_num*20+x_num*10):
    if np.mod(part_id+1, 5)!=0:
        exec(f"n = a.instances['Part-{part_id+1}-1'].nodes")
        exec(f"nodes{flag+1} = n.getSequenceFromMask(mask=('[#f]', ), )")
        flag += 1
...
# Define Job
    job = mdb.Job(name=f'RightLoadFreq=200Amp={force_id+1}hor_num={hor_num}', model='Model-1', description='', type=ANALYSIS, 
